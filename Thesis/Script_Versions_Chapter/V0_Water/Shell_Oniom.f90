  ...
  print *,Ndum
  do i=1,So_atoms !notice fixed format for input
     read(9,'(i3)',advance='no') bonds_So(i)
     !print*, bonds_So(i)
     if (bonds_So(i)>0) then
        do j=1,bonds_So(i)
           read(9,'(i3)',advance='no') link_So(i,j)
           !print*, link_So(i,j)
        end do
     end if
     read (9,*)
  ...
  do i=1,Sv_atoms
     read(9,'(i3)',advance='no') bonds_Sv(i)
     !print*, bonds_Sv(i)
     if (bonds_Sv(i)>0) then
        do j=1,bonds_Sv(i)
           read(9,'(i3)',advance='no') link_Sv(i,j)
           !print*, link_Sv(i,j)
        end do
     end if
     read (9,*)
  ...
  read(9,*) extra !how many extra intramolecular lines? (0=none)
  do i=1,extra
     read(9,'(a50)') intra(i)
     !print*, intra(i)
  end do
  ...
  do cf=1,N_conf !Loop over all configurations
     ...
     do i=1,So_atoms
        if (is_dummy(i) == 0) then ! do not write connectivity for dummy atoms
           ...
           if (bonds_So(i)>0) then
              do j=1,bonds_So(i)
                 write(11,'(i5,x,a4)',advance='no') link_So(i,j), "1.0"
                 write(12,'(i5,x,a4)',advance='no') link_So(i,j), "1.0"
                 write(13,'(i5,x,a4)',advance='no') link_So(i,j), "1.0"
              end do
           end if
           write (11,*)
           write (12,*)
     ...
     do i=2,cnt
        do ii=1,Sv_atoms
           write(11,'(i5)',advance='no') (i-2)*Sv_Atoms+So_Atoms+ii-1
           write(12,'(i5)',advance='no') (i-2)*Sv_Atoms+So_Atoms+ii-1
           write(13,'(i5)',advance='no') (i-2)*Sv_Atoms+So_Atoms+ii-1
           if (bonds_Sv(ii)>0) then
              do j=1,bonds_Sv(ii)
                 write(11,'(i5,x,a4)',advance='no') (i-2)*Sv_Atoms+So_Atoms-1+link_Sv(ii,j), "1.0"
                 write(12,'(i5,x,a4)',advance='no') (i-2)*Sv_Atoms+So_Atoms-1+link_Sv(ii,j), "1.0"
                 write(13,'(i5,x,a4)',advance='no') (i-2)*Sv_Atoms+So_Atoms-1+link_Sv(ii,j), "1.0"
              end do
           end if
           write (11,*)
           write (12,*)
