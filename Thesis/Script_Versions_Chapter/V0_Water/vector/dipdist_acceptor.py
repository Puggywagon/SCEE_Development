...
acceptors_min, acceptors_max = 0, 4           # adjust if needed
mu_min, mu_max = 1.65, 3.75       # reuse your limits

acceptors_bins = np.arange(acceptors_min - 0.5, acceptors_max + 1.5, 1.0)   # centred on integers
mu_bins = np.linspace(mu_min, mu_max, 20)                 # smoother in 
...
for p, colour in zip(p_list, color_list):
    ...
    # basic guard
    if df.empty or ("muL" not in df.columns) or ("acceptors" not in df.columns):
        continue

    # 2D histogram density: H has shape (len(acceptors_bins)-1, len(mu_bins)-1)
    H, xedges, yedges = np.histogram2d(
        df["muL"].to_numpy(),
        df["acceptors"].to_numpy(),
        bins=[mu_bins, acceptors_bins],
        density=True
    )
...
ax.set_ylim([acceptors_min - 0.5, acceptors_max + 0.5])
ax.set_xlim([mu_min, mu_max])

ax.set_ylabel(r"$n_{\mathrm{acceptors}}$", fontsize=fontsize)
ax.set_xlabel(r"$\mu$ / D", fontsize=fontsize)

ax.set_yticks(range(acceptors_min, acceptors_max + 1))
...
fig.savefig(f"../dipdist_acceptors_{T}.png")
fig.savefig(f"../dipdist_acceptors_{T}.pdf")
