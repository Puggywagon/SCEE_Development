...
################################################################################
    def plot_murho(self):
        ...
        csvfile = './Results/pcm_dipoles.csv'
...
################################################################################
        ...
        for T, color in zip(T_list, color_list):
            ...
            plt.plot(tmp['Density'], tmp['My Dipole'], ls='None', color=color, marker='o', label=r'$T='+f'{T}$ K')
            plt.plot(tmp['Density'], tmp['PMC Dipole'], ls='', color=color, marker='o',markerfacecolor='none', markeredgewidth=1.0)
...
################################################################################
        ...
        plt.savefig('./Figures/murho.png')
        plt.savefig('./Figures/murho.pdf')
        if (show):
            plt.show()
        else:    
            plt.clf()
