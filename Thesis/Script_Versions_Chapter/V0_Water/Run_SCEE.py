...
for i in replicas:
    ...
    for T in T_list:
        ...
        for p in P_list:
            ...
            print(f'Run completed: replica_{i}/{T}K/{p}Bar')
            exit_dir()
        exit_dir()
    exit_dir()
exit_dir()
...
dir_list = glob.glob('./Simulations/replica_*/298K/1.0Bar')
...
for rundir in dir_list:
    ...
    for index, row in df.iterrows():
        x = np.array([2.0732078, 2.79883053, 3.52445326]) #Check how to calculate these...
    ...
    df['muL'] = muL_list
    df.to_csv('Dipole.csv', index=False)
    ...
    for i in replicas:
        for T in T_list:
            for p in P_list:
                dirs=f'./Simulations/replica_{i}/{T}K/{p}Bar'
                dir_exp.append(dirs)
                if rundir == dirs:
                    fig=f'replica_{i}_{T}K_{p}Bar'
                else:
                    continue
      
    plt.savefig(f'./Figures/dipole_Histogram_{fig}.pdf')
    plt.savefig(f'./Figures/dipole_Histogram_{fig}.png')
    plt.close()
