...
################################################################################
    def Pre_Eq_System(self,system_title):
        ...
        # gmx grompp -f md.mdp -c argon_start.pdb -p argon.top
        gromacs.grompp(f='nvt.mdp', c='em1.gro', p=f'{system_title}.top', o=f'{system_title}.tpr', maxwarn=2)
...
######################################################
    def get_dipole_model_liquid(self,system_title):
        gromacs.environment.flags['capture_output'] = True
        input_str = '0\n'
        tmp = gromacs.tools.Dipoles(f=f'Simulations/replica_0/330.15K/1.0Bar/{system_title}_QMMM_md3',
                                s=f'Simulations/replica_0/330.15K/1.0Bar/{system_title}_QMMM_md3',
                                input=input_str)
        chk, dipole_output, stderr = tmp.run()
