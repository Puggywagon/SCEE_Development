...
nHB_min, nHB_max = 0, 4           # adjust if needed
mu_min, mu_max = 1.65, 3.75       # reuse your limits

nHB_bins = np.arange(nHB_min - 0.5, nHB_max + 1.5, 1.0)   # centred on integers
mu_bins = np.linspace(mu_min, mu_max, 60)                 # smoother in 
...
for p, colour in zip(p_list, color_list):
    ...
    # basic guard
    if df.empty or ("muL" not in df.columns) or ("nHB" not in df.columns):
        continue

    # 2D histogram density: H has shape (len(nHB_bins)-1, len(mu_bins)-1)
    H, xedges, yedges = np.histogram2d(
        df["nHB"].to_numpy(),
        df["muL"].to_numpy(),
        bins=[nHB_bins, mu_bins],
    )
...
ax.set_xlim([nHB_min - 0.5, nHB_max + 0.5])
ax.set_ylim([mu_min, mu_max])

ax.set_xlabel(r"$n_{\mathrm{HB}}$", fontsize=fontsize)
ax.set_ylabel(r"$\mu$ / D", fontsize=fontsize)
...
fig.savefig(f"../dipdist_nHB_{T}.png")
fig.savefig(f"../dipdist_nHB_{T}.pdf")
