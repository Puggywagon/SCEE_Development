...
################################################################################
    def run_md(self, MD='Vacuum',Gro_File,Topology_File,L,initial_molecules,solresnametop):
        L=L+1.7
        ...
        self.insert_Molecules(Gro_File,initial_molecules)
        self.Write_to_top(Topology_File,initial_molecules,solresnametop)
################################################################################
    def run_md(self, MD='Box',Topology_File):
        # gmx grompp -f md.mdp -c argon_start.pdb -p argon.top
...
################################################################################
    def run_md(self, MD='Production',mdpfile,HOMEDIR,Topology_File,T,p):
        ...
        self.process_trajectory(system_title)
        self.process_gro()
################################################################################
