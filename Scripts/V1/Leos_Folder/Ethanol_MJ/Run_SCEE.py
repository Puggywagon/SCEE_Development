...
################################################################################
def read_data():
    csvfile_list = glob.glob(f'Simulations/replica_*/*/*/Dipole_Calculations_MJ.csv')
    df = pd.DataFrame()
...
def write_filtered_data(k):
    ...
    for csvfile in csvfile_list:
        ...
        out_path = os.path.join(out_dir, 'Dipole_Calculations_filtered_MJ.csv')
...
def plot_dipoledist(Molecule,k, bin_width=0.25, x_ceil=10.0):
    ...
    plt.tight_layout()
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_old_MJ.png', bbox_inches="tight", dpi=300)
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_old_MJ.pdf', bbox_inches="tight", dpi=300)
    plt.close(fig)
    ...
    stats_df.to_csv(f'../figures/vectors/dipole_stats_{Molecule}UA_old_MJ.csv', index=False)
    print(stats_df.to_string(index=False))
######################################################
def read_filtered_data():
    csvfile_list = glob.glob(f'Simulations/replica_*/*/*/Dipole_Calculations_filtered_MJ.csv')
    df = pd.DataFrame()
    for csvfile in csvfile_list:
        tmp = pd.read_csv(csvfile)
        df = pd.concat([df, tmp])
...
def plot_dipoledist_filtered(Molecule, k, bin_width=0.25, x_ceil=10.0):
    ...
    ax.tick_params(axis='both', which='major', labelsize=12)
    plt.tight_layout()
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_filtered_old_MJ.png', bbox_inches="tight", dpi=300)
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_filtered_old_MJ.pdf', bbox_inches="tight", dpi=300)
    plt.close(fig)
    ...
    stats_df = pd.DataFrame(stats)
    stats_df.to_csv(f'../figures/vectors/dipole_stats_{Molecule}UA_filtered_old_MJ.csv', index=False)
    print(stats_df.to_string(index=False))
######################################################
...
for rundir in dir_list:
    ...
    df1.to_csv('Dipole_SCEE_MJ.csv', index=False)
    ...
    df.to_csv('Dipole_Calculations_MJ.csv', index=False)  
    os.chdir(HOMEDIR)  
...
collection.to_csv(f'../figures/vectors/Results_{central}UA_old_MJ.csv', index=False) 
print(f'Data saved in ../figures/vectors/Results_{central}UA.csv')
################################################################################  
