...
def plot_dipoledist(Molecule,k, bin_width=0.25, x_ceil=10.0):
    ...
    plt.tight_layout()
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA.png', bbox_inches="tight", dpi=300)
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA.pdf', bbox_inches="tight", dpi=300)
    plt.close(fig)
    ...
    stats_df.to_csv(f'../figures/vectors/dipole_stats_{Molecule}UA.csv', index=False)
...
def plot_dipoledist_filtered(Molecule, k, bin_width=0.25, x_ceil=10.0):
    ...
    ax.tick_params(axis='both', which='major', labelsize=12)
    plt.tight_layout()
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_filtered.png', bbox_inches="tight", dpi=300)
    plt.savefig(f'../figures/dipole_distribution_{Molecule}UA_filtered.pdf', bbox_inches="tight", dpi=300)
    plt.close(fig)
    ...
    stats_df = pd.DataFrame(stats)
    stats_df.to_csv(f'../figures/vectors/dipole_stats_{Molecule}UA_filtered.csv', index=False)
    print(stats_df.to_string(index=False))
######################################################
...
collection.to_csv(f'../figures/vectors/Results_{central}UA.csv', index=False) 
