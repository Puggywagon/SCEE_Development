...
def read_data():
    ...
    df2.columns = ['Time', 'Temperature', 'Pressure', 'Density','Epsilon','Number of Honds','Percentage of H-bonds','Nearest Neighbour','Dipole Moment','State']
    return df2
#################################################################################
def plot_density(df1,df2,T_list):
    ...
    for T, color in zip(T_list, color_list):
        ...
        for i in range(minlen,maxlen,1):
            if tmp['State'].iloc[i]=='liquid':
                plt.plot(tmp['Pressure'].iloc[i], tmp['Density'].iloc[i], ls='',
                     color=color, marker='o')
            elif tmp['State'].iloc[i]=='supercritical':
                plt.plot(tmp['Pressure'].iloc[i], tmp['Density'].iloc[i], ls='',
                    color=color, marker='o')
            else:
                plt.scatter(tmp['Pressure'].iloc[i], tmp['Density'].iloc[i], ls='',
                      marker='o', facecolor='none', edgecolors=color)
    ...
    plt.close()
#################################################################################
def plot_epsilondipole(df2,T_list):
    cmap = plt.cm.rainbow
    color_list = [cmap(x) for x in np.linspace(0.0, 1.0, len(T_list))]
    plt.figure(figsize=(15, 12))
    for T, color in zip(T_list, color_list):
        tmp = df2[df2['Temperature'] == T]
        plt.plot(tmp['Epsilon'], tmp['Dipole Moment'], ls='None',
                 color=color, marker='o',label=r'$T='+f'{T}$ K')

    plt.ylabel(r'Dipole Moment / D',fontsize=22)
    plt.xlabel(r'Dielectric Constant',fontsize=22)
    #plt.xscale('log')
    #plt.xlim([0.5, 1.0e4])
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)  
    ...
    Min=min(T_list)
    Max=max(T_list)
    plt.savefig(f'Dipole_Epsilon_{Min}_{Max}.pdf', dpi=300)
    plt.savefig(f'Dipole_Epsilon_{Min}_{Max}.png', dpi=300)
    plt.close()
#################################################################################
